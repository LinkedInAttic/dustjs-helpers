...
function filter(chunk, context, bodies, params, filterOp) {
  ...
  if (filterOp(coerce(expectedValue, params.type, context), coerce(actualKey, params.type, context))) {
    ...
    if (body) {
      ...
      // Logic to support @or - here if filterOp returned true; recorded as chunk.data =['t']
      if (context.current()._or) {
        if (!chunk.data.length) {
          chunk.data = ['t'];
        }
        return chunk;
      }
      _log("Missing body block in the " + filterOpType + " helper ");
      return chunk;
    }
  } else if (bodies['else']) {
...
var helpers = {
  ...
  */
  ...
  "or": function(chunk, context, bodies, params) {
    var origData,
      _chunkWrite,
      truth;

    if( bodies.block ) {
      // Capture true body and patch chunk.write to do nothing.
      // Otherwise whitespace around test helpers ends up in chunk
      // Save starting chunk data and reset data to empty.
      _chunkWrite = chunk.write;
      chunk.write = function () {return chunk;};
      chunk.w = chunk.write;
      origData = chunk.data;
      chunk.data = [];

      chunk.render(bodies.block, context.push({ isSelect: true, isResolved: false, isRendered: false, _or: true}));
      // Restore chunk.write, see if chunk.data[0] === t meaning true value computed
      // Restore chunk to what it was on entry. If truth found, render the true block
      chunk.write = _chunkWrite;
      chunk.w = chunk.write;
      truth = chunk.data[0];
      chunk.data = origData;
      if (truth === 't') {
        chunk = chunk.render(bodies.true, context);
      } else if (bodies.false) {
        chunk = chunk.render(bodies.false, context);
      }
      return chunk;
    } else {
     _log( "Missing body block in the or helper ");
    return chunk;
    }
  },
