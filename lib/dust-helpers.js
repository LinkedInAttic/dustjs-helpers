...
var helpers = {
  ...
  */
  ...
  /**
  * megaif helper for more flexible evaluation of complex logic expressions involving context's pieces of data wich don't need to have unambiguous string representations
  * example usage:
  * {@megaif cond="{0} == {1}" p0=objA p1=objB}
  *   Same object!
  * {:else}
  *   Different objects!
  * {/megaif}
  *   
  * @param cond, the condition to evaluate where every occurence of {<number>} will be replaced by the corresponding parameter
  * @param p<number>, the parameter to use instead of any references to {<number>} inside of the condition to evaluate
  */
  "megaif": (function () {
    var regEx = /{(\d+)}/g;
    var cachedEval = {};
    ...
    return function (chunk, ctx, bodies, params) {
        var cond = dust.helpers.tap(params.cond, chunk, ctx);

        if (typeof (cond) !== 'string') {
            _log(
				'cond should be a string'
			);

            return chunk;
        }

        if (!cachedEval.hasOwnProperty(cond)) {
            regEx.lastIndex = 0;
            ...
            while ((m = regEx.exec(cond)) != null) {
                properties[m[1]] = true;
                evalCond = evalCond.replace(m[0], "args[" + m[1] + "]");
            }

            var method = null;
            eval("method = function (args) { return " + evalCond + "; }");

            cachedEval[cond] = { method: method, properties: properties };
        }

        var _method = cachedEval[cond].method;
        var _properties = cachedEval[cond].properties;
        var args = {};

        for (var property in _properties) {
            var propertyName = 'p' + property;

            if (!params.hasOwnProperty(propertyName)) {
                _log('property ' + propertyName + ' is missing');

                return chunk;
            }

            args[property] = dust.helpers.tap(params[propertyName], chunk, ctx);
        }

        if (_method(args)) {
            if (bodies['block']) {
                return chunk.render(bodies.block);
            }
        ...
        if (bodies['else']) {
            return chunk.render(bodies.else);
        }
