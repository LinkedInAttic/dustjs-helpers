...
var helpers = {
  ...
  */
  ...
  /**
   * templateName helper
   * @param {string} output specifies where to write template name output.
   * Values can be "console" or "inline". Default is output.
   */
  "templateName": function(chunk, context, bodies, params) {
    params = params || {};
    var  output = params.output || 'inline';
    output = dust.helpers.tap(output, chunk, context);
    if (output === 'console') {
      _console.log('current template: ' + context.getTemplateName());
    } else if (output === 'inline'){
      chunk.write(context.getTemplateName());
    } else {
      dust.log('[dust-helpers] INVALID output type for helper [templateName] in template [' + context.templateName + ']', 'ERROR');
    }
    return chunk;
  },
  /**
