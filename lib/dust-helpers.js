...
var helpers = {
 ...
 /**
   iterate helper, loops over supplied object.
   Inspired: https://github.com/akdubya/dustjs/issues/9
   iterate helper uses a JavaScript for...in loop to iterate on the object passed as key.
   For an insight into what JavaScript does when something like a string, array, etc
   is passed, read: http://javascriptweblog.wordpress.com/2011/01/04/exploring-javascript-for-in-loops/
 
   Example:
   	@iterate key=obj}{$key}-{$value} of type {$type}{~n}{/iter}
 
   @param key - object of the iteration - Mandatory parameter
   	Three values are available in the iteration block: $key, $value, $type
        $key and $value are what you would expect. $type is the result of typeof on $value.
   @param sort - Optional. If omitted, no sort is done. Supported values:
	sort="asc" - sort ascending (per JavaScript array sort rules)
	sort="desc" - sort descending
	sort="fname" - Look for fname object in global context,
	if found, use it for the JavaScript array sort compare function.
	if not found, result is undefined (actually sorts ascending but you should not depend on it)
  **/
  "iterate": function (chunk, context, bodies, params) {
     ...
     function processBody(key, value) {
       return body(chunk, context.push({$key: key, $value: value, $type: typeof value}));
     };
  },
